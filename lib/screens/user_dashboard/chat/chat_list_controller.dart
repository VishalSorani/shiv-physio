...
class UserChatListController extends BaseController {
  ...
  UserChatListController(this._chatRepository, this._storageService);
  ...
  void onInit() {
    super.onInit();
    // Track screen view
    trackScreenView('user_chat_list_screen');
    // Only load conversations, don't auto-navigate
    loadConversations();
    _listenToConversations();
    _loadUnreadCount();
  }

  /// Get or create conversation with doctor when user explicitly taps on chat
  /// This is called from the dashboard controller when user taps the Messages tab
  Future<void> initializeDoctorConversation() async {
    try {
      // Get or create conversation with doctor
      final conversation = await _chatRepository.getOrCreateDoctorConversation();
      
      // Convert to BaseConversationModel for navigation
      final baseConversation = await _chatRepository.getConversation(conversation.id);
      ...
      // Track analytics
      trackAnalyticsEvent('doctor_conversation_opened', parameters: {
        'conversation_id': conversation.id,
      });
    } catch (e) {
      log(
        'Error initializing doctor conversation: $e',
        name: 'UserChatListController',
      );
      // If error, just show the chat list (user can manually start conversation)
    }
  }
